...
const Dashboard = () => {
  ...
  const {data:books, error, isPending} = useFetch(`http://localhost:3000/api/v1/users/${user.id}/books`, "GET");
  ...
  return (
    <section id="dashboard" className={styles.dashboard}>
      {isPending && <Loader />}
      {books?.length > 0 ? <BookRow books={books}/> : <h2>You don't have any book in your library.</h2>}
